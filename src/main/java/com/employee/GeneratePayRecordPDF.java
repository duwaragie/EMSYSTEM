...
@WebServlet("/GeneratePayRecordPDF")
public class GeneratePayRecordPDF extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (payrollList == null || payrollList.isEmpty()) {
            ...
            response.getWriter().write("<script type='text/javascript'>alert('No payroll records found.');window.location.href='ReadPayRecordServlet';</script>");
            return;
        }

        // Specify the server-side location where the PDF will be saved
        String filePath = "C:\\Users\\Zayan Mohamed\\eclipse-workspace\\EmployeeManagment\\src\\main\\webapp\\WEB-INF\\reports\\payroll_records_report.pdf";
        ...
        try (FileOutputStream fos = new FileOutputStream(filePath)) {
            ...
            response.setContentType("text/html");
            response.getWriter().write("<script type='text/javascript'>alert('Payroll records report generated successfully!'); window.location.href='ReadPayRecordServlet';</script>");
        } catch (DocumentException e) {
            // Catch DocumentException and notify failure
            response.setContentType("text/html");
            response.getWriter().write("<script type='text/javascript'>alert('Failed to generate the payroll records report. Please try again.'); window.location.href='ReadPayRecordServlet';</script>");
        } catch (IOException e) {
            // Catch IOException and notify failure
            response.setContentType("text/html");
            response.getWriter().write("<script type='text/javascript'>alert('Error saving the report. Please check the file path.'); window.location.href='ReadPayRecordServlet';</script>");
        }
    }
}
