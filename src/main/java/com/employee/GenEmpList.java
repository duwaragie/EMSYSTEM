...
@WebServlet("/GenEmpList")
public class GenEmpList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (employeeList == null || employeeList.isEmpty()) {
            ...
            response.getWriter().write("<script type='text/javascript'>alert('No employee records found.');window.location.href='ReadEmployeeServlet';</script>");
            return;
        }

        // Specify the server-side location where the PDF will be saved
        String filePath = "C:\\Users\\Zayan Mohamed\\eclipse-workspace\\EmployeeManagment\\src\\main\\webapp\\WEB-INF\\reports\\employee_details.pdf";
        ...
        try (FileOutputStream fos = new FileOutputStream(filePath)) {
            ...
            String[] headers = {"Employee ID", "Name", "Email", "Phone", "Hire Date", "Job Title","Username"};
            ...
            response.setContentType("text/html");
            response.getWriter().write("<script type='text/javascript'>alert('Employee details report generated successfully!'); window.location.href='ReadEmployeeServlet';</script>");
        } catch (DocumentException e) {
            // Catch DocumentException and notify failure
            response.setContentType("text/html");
            response.getWriter().write("<script type='text/javascript'>alert('Failed to generate the employee details report. Please try again.'); window.location.href='ReadEmployeeServlet';</script>");
        } catch (IOException e) {
            // Catch IOException and notify failure
            response.setContentType("text/html");
            response.getWriter().write("<script type='text/javascript'>alert('Error saving the report. Please check the file path.'); window.location.href='ReadEmployeeServlet';</script>");
        }
    }
}